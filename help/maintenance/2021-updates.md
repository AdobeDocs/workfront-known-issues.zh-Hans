...
description: '[!DNL Adobe Workfront]的2021年维护更新历史记录'
...
当用户登录 [!DNL Workfront Proof] 查看文件夹时，文件夹显示为空。 如果用户稍后再检查，则校样可见。
...
当[!UICONTROL 工作负载均衡器]中的用户使用[!UICONTROL 分配:Role ID]字段创建过滤器时，[!UICONTROL 未分配的工作]区域未加载。
...
当用户查看具有多列的看板展示板时，列太宽，用户必须滚动才能查看或将信息卡移动到最右边的列。 列宽不可调整，因此用户无法将列缩小，使它们同时在屏幕上可见。
...
当用户打开打印摘要以打印校样时，标题会出现，但摘要本身是空白的。
...
当用户尝试在旧版 [!DNL Workfront] Experience 中上载新校样或新版本校样时，新校样或新版本页面为空白，用户无法上载校样或版本。

+++

+++**2021 年 7 月 29 日维护更新**

**[!UICONTROL 校样活动]和[!UICONTROL 校样查看者设置]页面始终可用**

_[!DNL Workfront Proof]_

[!UICONTROL 校样活动]和[!UICONTROL 校样查看者]设置页面现在始终可见，即使用户无权更新这些页面。

以前，当具有自定义校样权限配置文件的用户导航到文档时，左侧的[!UICONTROL 校样活动]和[!UICONTROL 校样查看者设置]页面并不总是可见。
...
当用户在 [!DNL Adobe Workfront Classic] 上载文件名中包含句点的新版本校样时，文件名在末尾被截断。
...
**[!UICONTROL 前往校样]链接不指向评论**

_电子邮件通知_

当用户收到校样评论的电子邮件通知并单击[!UICONTROL 前往校样]时，用户被定向到校样中的错误评论，或者根本没有被定向到任何评论。
...
当新 [!DNL Adobe Workfront] Experience 用户创建新校样，并且在校样的某些自定义字段中输入值时，一些之前字段的值恢复为默认值，而不是用户输入的值。
...
当新 [!DNL Adobe Workfront] Experience 用户在团队设置中设置特定的信息卡颜色时，这些颜色未反映在看板信息卡上。

**无法在自定义消息字段中粘贴文本**

_[!DNL Workfront Proof]_

当用户在 [!UICONTROL Web 校对查看者]中创建新校对并且试图将文本粘贴到[!UICONTROL 电子邮件通知]分区中的[!UICONTROL 消息]字段时，无法粘贴文本。 这似乎只在文本具有段落格式并且有段落分隔符时发生。
...
当用户在新版 [!DNL Adobe Workfront] Experience 尝试扩展附加到项目的自定义表单时，他们无法打开自定义表单，且看到错误消息“[!UICONTROL 发生错误，我们正在努力解决该问题。 要继续您的工作，请尝试刷新此浏览器页面。]” 刷新页面并不能解决问题。
...
当新 [!DNL Adobe Workfront] Experience 用户添加自定义[!UICONTROL 附加字段]列到看板展示板时，所有列标题停止显示在展示板上。
...
**[!UICONTROL 新校样]页面上的一些短语翻译不正确**

_[!DNL Workfront Proof]_

当用户导航到 [!DNL Workfront Proof] 中的[!UICONTROL 新校样创建]页并且页面内容被翻译成英语以外的其他语言时，一些短语仍以英语显示。
...
当新版 [!DNL Workfront] Experience 用户尝试打开某些对象时，他们会看到错误消息“[!UICONTROL （对象）不再存在：您可能输入了错误的网址。 请多次检查并再次尝试输入地址。]“ 对象链接仍然出现在列表、最近打开文件、收藏夹、搜索结果等中，但他们无法访问它，并且链接未出现在已删除对象的“回收站”中。
...
当用户在 [!DNL Workfront] 中上载其他版本校对时，[!UICONTROL 公开校对]和[!UICONTROL 校对工作流]链接缺失，使其看起来像是文档而不是校对。 当这些链接缺失时，[!UICONTROL 待办]标签仍然显示，并且处于[!UICONTROL 待办]状态下，其他用户无法生成校样。
...
当用户在 [!DNL Workfront Proof] 中创建新校样并且试图将文本从电子邮件粘贴到[!UICONTROL 电子邮件通知]分区的[!UICONTROL 消息]字段时，用户无法粘贴文本。
...
当用户创建新版本的校样并更新校样名称时，[!DNL Workfront] 中文档对象保留原始名称而不是匹配新的校样名称。
...
当用户在[!UICONTROL 主页]或者[!UICONTROL 我的工作]查看工作列表时，已存档的校样出现在列表中。 已存档的校样同样也出现在报告和仪表板上。
...
[!DNL Workfront] 现在只发送一封电子邮件用于合并校样，而不是为包含的每个文件发送一封邮件。
+++
...
**无法在 [!DNL Adobe Creative Cloud] 集成中创建校样**

_Workfront 集成_

当用户尝试直接从 [!DNL Adobe Creative Cloud] 创建校样时，校样未生成。
...
当在默认校样设置中启用[!UICONTROL 公共共享]的用户创建校样时，将在禁用共享的情况下创建校样。 其他用户无法看到[!UICONTROL 共享]按钮或共享校样。

**创建校样时出现“[!UICONTROL 无法生成校样]”错误**

_[!DNL Workfront Proof]_

当用户尝试创建校样时，校样未创建，并且用户会看到以下错误消息：

“[!UICONTROL 校样未能生成——内部错误]”
...
当用户使用[!DNL Workfront] API中的术语来选择要包含在[!DNL Workfront]模块输出中的集合或引用字段时，该模块的输出显示带有冒号的字段（例如[!UICONTROL 所有者:name]），以及属性中的字段（名称是所有者下的字段）。 标有冒号的字段不包含数据，如果稍后在场景中映射到模块，则会提供不正确的数据。
...
如果独立 [!DNL Workfront Proof] 帐户移至共享组件，当用户添加新版本的校样或文档时，可能会出现以下功能：

* 用户无法删除用户[!UICONTROL 工作室校样]。
* 默认消息未出现在新版本上。
...
当用户在校样上启用公共链接共享然后上传新版本的校样时，新版本的校样未自动启用公共链接共享。

**[!UICONTROL 审批]、[!UICONTROL 更改]和[!UICONTROL 拒绝]按钮在校样中缺失**

_[!DNL Workfront Proof]_

当用户在校样查看者中查看校样时，屏幕顶部缺失[!UICONTROL 审批]、[!UICONTROL 更改]和[!UICONTROL 拒绝]按钮。
...
当用户将自定义消息附加到校样，然后上载该校样的新版本时，自定义消息未出现在新校样上。
...
**无论工作流设置如何，“[!UICONTROL 将此校样通知收件人]”选项都已禁用**

_[!DNL Workfront Proof]_

当用户创建新校样，并且没有手动打开“[!UICONTROL 将此校样通知收件人]”选项时，则不会通知预期的收件人。 即使在工作流设置中启用了该选项，情况也是如此。
...
当用户从[!UICONTROL 校样查看者]下载文档时，文档的名称是文档的先前版本名称，而非下载的版本名称。
...
当用户在校样评论中被标记并单击电子邮件通知中的[!UICONTROL 前往校样]链接时，被定向到校样而不是特定评论。 如果用户在 [!DNL Workfront Classic] 中，则被定向到[!UICONTROL 文档详情]页面而不是校样中的评论。

**添加到 [!DNL Workfront] 阶段的用户收到电子邮件通知**

_[!DNL Workfront Proof]_

当不在工作流中的用户打开 [!DNL Workfront] 中的校样时，系统自动创建一个阶段，将此用户添加到校样中，并发送[!UICONTROL 新校样]电子邮件通知。
...
当用户创建新校样，并且启用“[!UICONTROL 将此校样通知收件人]”选项时，未发送电子邮件通知收件人。
...
当新版 [!DNL Workfront] Experience 用户单击某些文档链接时，他们无法访问这些文档并看到错误消息“[!UICONTROL 文档不再存在：您可能输入了错误的网址。 请多次检查并再次尝试输入地址。]“ 验证电子邮件通知中的[!UICONTROL 查看详细信息]链接也存在同样的错误。
...
当用户打开文件夹并添加新校样明时，该校样上载到对象的一般[!UICONTROL 文档]区域而非文件夹内。
...
**“[!UICONTROL 允许通过 URL 或嵌入代码共享校样]”设置更改**

_[!DNL Workfront Proof]_

当用户创建校样并取消选中[!UICONTROL 允许通过 URL 或嵌入代码共享校样]设置时，生成校样后再次选中设置。 如果用户选中设置，则在生成校样后取消选中。

**[!DNL Mac]用户无法粘贴到校样查看者中的文本字段中**

_[!DNL Workfront Proof]_

当用户尝试将文本粘贴到校样查看者中的某些字段时，该文本不会出现在该字段中。
...
当用户尝试在新版 [!DNL Workfront] Experience 移动文档或校样时，项目外的一些任务没有按预期列出父项目。
