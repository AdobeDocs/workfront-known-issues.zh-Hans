...
计算字段中的 **[!UICONTROL HOUR] 函数使用 UTC**
...
>[!NOTE]
>
>此问题已于 2022 年 10 月 13 日在预览环境中修复。已于2022年10月27日在生产中修复。

当用户在某个图钉上选择[!UICONTROL 移除图钉]选项，收到有关移除的消息，并尝试通过单击消息中的[!UICONTROL 撤消]来替换该图钉时，图钉不会在顶部导航中被替换，也不会被添加到[!UICONTROL 更多图钉]列表（[!UICONTROL 图钉]区域中的三个点菜单）下的图钉列表中。

如果用户尝试通过转到页面并固定它来重新固定页面，则不会创建图钉，并且用户无法固定页面。

**在 [!DNL Safari] 浏览器中使用可共享链接时，[!UICONTROL 工作负载均衡器]中列出所有用户**

*[!UICONTROL 工作负载均衡器]*

当用户在使用 [!DNL Safari] 浏览器过程中点击[!UICONTROL 工作负载均衡器]的可共享链接时，他们会看到所有用户，而不只是列出的团队成员。
...
当用户尝试在[!UICONTROL 资源规划程序]中为新过滤器命名时，名称框保持空白。此外，如果用户已按下空格键，则[!UICONTROL 保存]按钮将禁用。

**无法编辑任务或问题的名称或完成百分比**

*任务和问题*

具有对任务或问题的[!UICONTROL 参与]访问权限的用户无法在标题中编辑任务或问题的名称。此外，具有[!UICONTROL 参与]访问权限的用户无法编辑任务或问题的完成百分比。
...
当正在 [!DNL Workfront] 中查看验证的用户关闭验证时，他们不会返回在打开验证之前所处的页面。相反，他们会被重定向到 [!DNL Workfront] 中的另一个页面。
...
使用版面模板时，我们已将[!UICONTROL 预计开始日期]和[!UICONTROL 预计完成日期]添加到可自定义项目标题的字段列表中。
...
当您使用版面模板时，您现在可以添加[!UICONTROL 集成]字段到项目的自定义标题中。 添加后，该字段将显示链接到该项目的外部项链接，该项目位于 [!DNL Salesforce] 或者 [!DNL Anaplan]，具体取决于您的集成。
...
当您将新字段添加到可自定义的标题时，该字段现在添加为标题左侧的第一个字段，或者紧跟在[!UICONTROL 搜索]版面模板内的框后。在此更改之前，该字段添加为标题中的最后一个字段。
...
当用户查看对象的[!UICONTROL 自定义]分区时，自定义分区显示不属于该对象的项。 当自定义分区直接添加到对象时，以及通过版面模板添加自定义分区时，已报告此问题。
...
当用户通过已固定为版面模板一部分的选项卡进入项目页面时，该页面默认为左侧导航的[!UICONTROL 所有项目]区域。 即使用户选择左侧导航的另一个区域，然后导航离开“项目”页面并返回，也会发生这种情况。
...
用户报告说，当[!DNL Workfront]管理员使用版面模板为某些对象自定义术语时，新对象名称在界面中显示不一致。
...
当用户查看[!UICONTROL 我的工作]列表，并且[!UICONTROL 我的工作]列表的版面模板包含[!UICONTROL 完成百分比]等数值时，该[!UICONTROL 我的工作]列表不显示。
