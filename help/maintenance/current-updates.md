...
当用户回复评论时，其他应收到该回复通知的用户未收到这些通知。一些用户可能会收到通知，但另一些用户则未收到通知。
...
当用户查看验证并单击[!UICONTROL 验证活动]选项卡时，该选项卡使用户返回[!UICONTROL 校对详细信息]选项卡。

在将其他用户分配给任务时，会重新分配&#x200B;**[!UICONTROL 计划小时数]**

_任务_

在将用户分配给一个其[!UICONTROL 计划小时数]已分配给其他被分派人的任务时，该任务的[!UICONTROL 计划小时数]将平均分配给该任务的所有被分派人。
...
对于从未在展示板应用程序中创建工作流的帐户，已从展示板仪表板中删除工作流区域。使用工作流的帐户仍有权访问它们。
