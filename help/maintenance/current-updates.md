...
当用户尝试在 Web 验证查看器中查看验证时不显示验证，并且用户看到以下错误：
...
当用户尝试创建验证的新版本时并不创建新版本，并且用户看到以下错误消息：
...
当用户对验证发表评论时，该验证还产生另一条评论，而该评论为空。

已在视频验证上报告了此情况。

**[!UICONTROL 验证活动]选项卡不打开**
...
已在从 URL 创建验证时报告了以下问题。

* 验证似乎褪色。
* 验证被裁切。

这可能导致难以制定验证决策，因为未准确地表示验证。

**生成验证耗时很长**

_验证_

当用户尝试生成验证时，生成验证耗时很长。生成验证可能耗时数日。
...
“[!UICONTROL 出现以下错误：找不到身份验证类型 null 的工厂]”
...
当用户在验证查看器中查看验证并尝试选择列表或项目符号中的文本时，文本选择工具不起作用，无法选择文本。
...
当用户在验证查看器中查看验证并尝试使用折线工具发表评论时，该工具无法标出该验证。
...
当一个用户在[!UICONTROL 验证查看器]中将另一个用户添加到验证时，无法打开“[!UICONTROL 验证角色]”和“[!UICONTROL 电子邮件警报]”下拉列表。该用户无法分配验证角色或电子邮件提醒。当通过评论添加用户或与用户共享验证时，可能会发生这种情况。
