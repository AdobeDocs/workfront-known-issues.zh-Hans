...
将新用户导入Workfront时，对话框窗口中缺少“向这些用户发送邀请电子邮件”复选框。
...
当用户分配任务并开始键入在任务期间计划了休息时间的被分配人的名称时，建议分配区域下的用户图标不显示休息时间（飞机）图标。 如果用户继续输入，以使名称在“用户和团队”区域下显示，则会显示该图标。
...
**“标记为已完成”关闭我的问题构件的摘要窗格**

当尝试使用通过我的问题构件打开的问题摘要窗格中的“标记为完成”按钮时，摘要窗格会意外关闭。

**布局模板构件设置无法添加列**

通过布局模板构件设置选择要添加到构件的字段时，可以选择字段，但相应的列不会出现在构件中。
