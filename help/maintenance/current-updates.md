...
* 现在，当您选中列表中的复选框时，它们会显示焦点指示器。 这样，列表中项目的键盘导航便于直观地跟踪。
* 列表工具栏中的所有按钮现在具有一致的悬停状态和焦点状态，在悬停时显示灰色背景，在焦点上显示灰色背景和蓝色轮廓。
* 以前，在使用空格键在列表中打开下拉菜单时，将会打开该菜单，并且页面也会向下滚动一点，这不是本意。 现在，在下拉框上按空格键时，页面不再滚动，这是正确的预期行为。
* 现在，在查看选中行复选框的列表时，您可以在每个可编辑项目中执行制表符操作，然后按空格键切换到编辑模式，并开始在行中编辑该单元格。 以前，这些项目不能通过键盘导航，需要使用鼠标。 现在，切换到编辑模式时，鼠标和键盘都很友好。

**&quot;[!UICONTROL 哇]从模板创建项目时出错**
...
**为时间表中灰显的小时单元格添加的工具提示**

*时间表*

我们添加了工具提示，以解释工时单中某个小时单元格可能变暗的原因。 例如，部分原因可能是因为时间表已关闭或项目已完成。
...
**工时单、工时单配置文件和工时单首选项的新信息图标**

*Workfront*

>[!NOTE]
>
>此更新仅发布到预览环境。 它将随23.1版本一起发布到生产环境。

我们在以下设置中添加了多个信息图标：

* 创建或编辑工时单或工时单配置文件时，“可以编辑时间”复选框，以指示在启用时，批准者还可以提交、重新打开或编辑工时单，除非您的管理员在“设置”的“工时单首选项”区域中限制这些操作。
* “设置”的“时间表和小时首选项”区域中的“将时间表编辑限制为所有者和管理员”，以指明在禁用时，以下用户还可以编辑工时单：具有时间表和小时数管理访问权限的用户、允许编辑时间的时间表批准者以及时间表所有者的经理。

请注意，这些设置的功能没有发生更改，而且只添加了信息图标，以便为设置范围增加清晰度。
