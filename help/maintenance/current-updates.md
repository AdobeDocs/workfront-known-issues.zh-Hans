...
+++**（已计划）2023 年 10 月 12 日维护更新**

**问题转化为任务时，计算字段未保留值**

_自定义表单_

将问题转换为任务时，引用自身的计算字段不会保留其值。

将问题转化为任务时，所需的值会在编辑窗口中正确显示。但是，转化完成后，计算字段显示“不适用”。

**在中更改筛选器时出错 [!UICONTROL 主页]**

_主页_

当用户更改过滤器时 [!UICONTROL 主页]， [!UICONTROL 主页] 区域未加载，且用户看到以下错误消息：
...
当用户加载展示板时，展示板的加载速度极慢。即使展示板具有少量信息卡，也可能会发生这种情况。

已存档的信息卡（即使未显示）会影响展示板加载时间。

**无法在列之间移动卡片**

_展示板_

当用户尝试移动展示板上的信息卡时，信息卡不移动。在以下情形下会发生这种情况：
...
用户可以将一张卡片从引入列拖到板上的另一列中，但后续的卡片不能从引入列中移出。

**分组方式影响董事会表现**

_展示板_

当用户尝试按被分配者或标记对信息卡进行分组时，展示板性能变得非常慢。

**不发送自动提醒电子邮件**

_通知_

不发送自动电子邮件提醒。此情况最初发生于 2023 年 9 月 14 日。
...
当用户在新的评论体验中发表评论，并且该评论显示在新体验的评论区域中时，同一评论可能不会出现在旧版评论体验中。这可能会导致使用旧版体验的用户错过评论。

**对象页面缺少元素**

_Workfront_

当用户导航到 [!DNL Workfront] 中某个对象的自定义部分时，加载的页面可能会缺少某些元素。这些元素可能包括：
...
当用户尝试将另一个用户分配给属于工作流的展示板中的任务，并开始键入该用户的名称时，该用户不会显示在可用用户的下拉列表中。即使用户处于活跃状态并且是展示板和工作流的成员，也会发生这种情况。

用户还可能会发现下拉列表中显示了已停用的用户。

**无法删除清单项目**

_展示板_

当用户尝试从展示板上的信息卡中删除清单项目时，[!UICONTROL 删除]按钮不响应，并且该项目未被删除。

**自定义表单加载缓慢**

_自定义表单_

当用户尝试加载自定义表单时，自定义表单加载缓慢。

**无法将文档移动到其他文件夹**

_文档_

当用户将文档移入对象文件夹中时，随后无法将该对象移动到其他文件夹。
...
当用户尝试从生产之外的环境下载文档时，该文档未上载，并且用户看到以下错误：
...
当用户尝试生成验证时，生成验证需花费过多的时间。验证生成可能需要几天时间。
...
当用户尝试从外部源添加文档时，[!DNL Workfront] 无法访问源，并且用户看到以下错误：

“[!UICONTROL 出现以下错误：找不到验证类型 null 的工厂]”
...
默认情况下，个人任务不再显示在时间表上。当个人任务被固定或有工时记录时，这些任务会显示在时间表上。在进行此更改之前，个人任务默认显示在时间表上。
