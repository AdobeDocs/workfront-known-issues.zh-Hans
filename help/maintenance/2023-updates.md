...
+++**[!DNL Adobe Workfront Fusion]2023 年 12 月 21 日维护更新**
...
当用户配置某个模块，而该模块需要从所连接的帐户获得数据（如为了选择一条记录）时，该模块无法检索数据，并且对数据的请求超时。
...
**审批卡在[!UICONTROL 正在等待我的审批]小组件**&#x200B;上

_主页_

当上传文档的新版本但未完成以前版本的审批时，该文档的旧版本卡在审批者的[!UICONTROL 正在等待我的审批]小组件上。 由于存在新版本，因此无法通过该审批，并且无法从该小组件中删除它。

**在任务或问题的展示板视图中添加工作项时出现问题**
...
**任务未显示在[!UICONTROL 我的工作]小组件中**

_[!UICONTROL 主页]_

当用户在[!UICONTROL 主页]中查看其[!UICONTROL 我的工作]小组件时，分配给该用户的某些任务未显示在该小组件中。 例如，用户进入某个项目可能会看到在该项目中为其分配了任务，但这些任务未显示在该用户的[!UICONTROL 我的工作]小组件中。
...
**利用率报告中的预算小时与通过 API 报告的预算小时不符**

_报告_

当用户通过 API 调用给定项目的 RPBGHR 对象，并比较该调用的结果与该项目的利用率报告时，结果不符。

**在“新请求”页面上显示的自定义货币不正确**

_请求_

当用户提交请求并作出选择以改变请求表单上的显示逻辑时，所显示的货币恢复为默认货币，而非在请求队列所代表的项目上设置的自定义货币。

当用户提交其请求时，该货币显示为适合请求队列所代表的项目的自定义货币

**通过 API 或[!DNL Workfront Fusion]** 发表的评论中存在额外的行
...
如果“我的工作”小组件在新主页的顶部，则它可获得所有预期的对象。 但是，如果此小组件在该页面上的任何其他小组件下，则它只能获得 10 个对象。
...
当用户在新[!UICONTROL 主页]体验的[!UICONTROL 待办事项]小组件上创建任务时，该任务显示在用户的时间表上。 即使该任务未记录小时数并且未固定个人项目，也发生这种情况。

+++

+++**2023 年 11 月 3 日维护更新（热修复）**

**将子任务移至父任务下方时，子任务的显示顺序混乱**
...
**过滤器不适用于新的[!UICONTROL 主页]体验**&#x200B;中的小组件

_[!UICONTROL 主页]_

当用户将过滤器应用于新[!UICONTROL 主页]体验中的小组件时，该小组件显示该过滤器应排除的项。

已在“自定义沙盒”环境中报告过此问题。 “预览”和“生产”环境中相同的小组件按预期的那样过滤。
...
当新[!UICONTROL 主页]体验中的用户尝试从[!UICONTROL 待办事项]小组件中删除一个项目时，该项目不会被删除，并且用户会看到以下错误：
...
此更新仅包括次要或不太突出的错误修复。[!DNL Workfront] 当您提交的问题得到修复时，支持人员会通知您。
...
_报告_
当用户运行任务报告时，报告在加载时似乎正确排序，但在加载结束时，用户看到报告未正确排序。
...
现在，如果您在撰写附加了图像的消息期间离开更新页面，该消息和图像会在您返回时保留。 在本次更新之前，未提交的评论会被保留，但图像会被删除。 目前这在目标的更新部分中，以及在启用评论测试版体验时出现的问题的“更新”部分中可用。
...
>[!NOTE]
>
>2022 年 11 月 3，此更新仅发布到了“预览”环境，但现在已在生产环境中提供。
...
当拥有工作、审阅或请求许可的用户尝试编辑自己的用户设置时，打开的弹出窗口为空白，用户无法进行任何编辑。 要退出弹出窗口，用户必须刷新页面。
...
**[!UICONTROL 工作负载平衡器]显示与项目/任务/问题无关的小时数**

_[!UICONTROL 工作负载平衡器]_

当用户查看[!UICONTROL 工作负载平衡器]时，将看到为用户记录的与任何项目、任务或问题无关的小时数，并且不将这些小时数记录为[!UICONTROL 常规]小时数。 这些小时数只能显示在 4 周或 6 周视图中。
...
>[!NOTE]
>
>此更新已于 2022 年 11 月 3 日发布到“预览”环境，现在在生产环境中提供
