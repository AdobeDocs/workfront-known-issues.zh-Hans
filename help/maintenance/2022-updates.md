...
“[!UICONTROL 错误。 抱歉！ 只有拥有计划许可证的用户才能批准或拒绝时间表。 请联系您的系统管理员。]”
...
当用户查看看板团队的积压事项时，一个或多个任务显示积压事项顺序为 0。
...
当用户尝试打开交互式校样时，该校样未打开，并且用户看到以下消息：

“[!UICONTROL 未加载校样 (501)，请重试]”
...
当用户查看校样并尝试从[!UICONTROL 共享]选项卡共享校样时，[!UICONTROL 新校样收件人]文本框非常小。 尽管用户可以键入名称，但由于此框太小，文本会以一种难以阅读的方式换行。
...
当用户作为查看者或请求者添加到校对时，他们将获得“[!UICONTROL 访客]”权限配置文件，该配置文件不应使用 [!DNL Workfront Proof] 许可证。 但是，当添加用户时，使用的 [!DNL Workfront Proof] 许可证的数量会增加。
...
当正在 [!DNL Workfront] 中查看校样的用户关闭校样时，他们不会返回到打开校样之前所处的页面。 相反，他们会被重定向到 [!DNL Workfront] 中的另一个页面。

**无法在[!DNL Workfront]** 中打开校样

*校样*

当用户在 [!DNL Workfront] 中查看文档并尝试打开校样时，校样未打开，并且用户返回到[!UICONTROL 文件详情]页面。
...
当用户查看具有多个页面的校样时，用户会看到页面的缩略图，但这些页面不会在主查看器中打开。
...
创建新校样时，如果用户上传大于 1.5GB 的文件，则文件名将变为红色，并阻止创建校样。
...
**“[!UICONTROL 添加到开发周期]”和“[!UICONTROL 添加到看板展示板]”选项在内联编辑任务时不可用&#39;**

*报告*

当用户查看报告中的任务列表并打开[!UICONTROL 更多]（三个点）菜单时，“[!UICONTROL 添加到开发周期]”和“[!UICONTROL 添加到看板展示板]”选项在下拉菜单中不可用。 如果在仪表板中查看报告，则“[!UICONTROL 添加到开发周期]”和“[!UICONTROL 添加到看板展示板]”选项在下拉菜单中可用。
...
当用户查看交互式校样并单击链接或包含链接的按钮时，用户不会转到链接或按钮链接到的页面。
...
当用户在校样评论中被标记时，他们不会收到有关评论的电子邮件通知。
...
**创建、移动或存档校样时，[!DNL Workfront Proof] 区域不会更新**

*[!DNL Workfront]Proof*

校样目前正在经历索引延迟。 这可能会以下列方式影响用户体验：
...
本月早些时候，作为 [!DNL Workfront] 生产环境补丁的一部分，我们修复了校样电子邮件通知系统的一些错误。 此更改在发布时未在维护更新中传达。 我们已将以下信息添加到 [2022 年 6 月 2 日维护更新](#maintenance-update-on-june-2-2022)：

由于这些错误修复，用于发送校样通知的电子邮件地址已更改。

之前，校样电子邮件地址包含您组织的子域。 例如，通知 @[公司域名].my.workfront.com

现在，校样电子邮件地址不再包含组织子域。 所有校样电子邮件通知都将来自以下地址：`notification@my.workfront.com`
...
* “[!UICONTROL 通过标记共享校样]”功能
* 校对角色（[!UICONTROL 审阅者]、[!UICONTROL 审批者]等）
...
当用户查看校样的[!UICONTROL 校样工作流]并尝试将自己拖到校样的其他阶段时，用户的名称会快速恢复到原始阶段，并且不会添加到所需阶段。
...
当用户尝试编辑 [!DNL Workfront Proof] 中用户角色的“[!UICONTROL 解决评论并应用操作]“或者“[!UICONTROL 通过标记共享校样]”权限时，更改未保存。 用户收到模板已更新的通知，但如果用户再次打开角色权限，他们可以看到更改未保存。
...
**无法从[!DNL PowerPoint]** 中创建校样

*[!DNL Workfront Proof]*

当用户尝试从包含图表的 [!DNL PowerPoint] 中创建校样时，校样创建失败。

**无法从 [!UICONTROL Word] 文档创建校样**

*[!DNL Workfront Proof]*

当用户尝试从包含图表的 [!DNL Word] 文档中创建校样时，校样创建失败。

**共享校样时无法添加自定义消息**

*[!DNL Workfront Proof]*

当用户查看校样时，打开[!UICONTROL 共享校样]区域，并选择[!UICONTROL 添加自定义消息]按钮时，用户无法在打开的文本框中键入内容。 当用户尝试在此框中键入内容时，该框立即消失。

**无法关闭校样**

*[!DNL Workfront Proof]*

当用户查看校样并尝试关闭它时，校样右上角的 X 缺失。
...
当用户尝试创建校样时，选项或按钮的文本不可见。 因此，用户不知道每个选项或按钮代表什么，也无法配置校样。
...
现在，只需在[!UICONTROL 工作负载平衡器]单击任务栏或问题栏，即可打开“摘要”面板。 在此更新之前，您必须单击工具栏中的[!UICONTROL 打开摘要]图标，然后单击任务或问题。 这种经历确实令人困惑，现在已得到纠正。 或者，您可以单击任务或问题名称旁边的[!UICONTROL 更多]菜单，然后单击[!UICONTROL 打开摘要]。
...
当用户在校样上进行评论且尝试打开[!UICONTROL 文本]工具时，工具未打开，且用户看到以下错误消息：

“[!UICONTROL 此页面的文本数据仍在下载中。 请稍候。]”

**校样电子邮件将发送到用户的主要电子邮件地址**

*[!DNL Workfront Proof]*

我们正在调整校样电子邮件通知的发送方式。 现在，通知将发送到用户的主要电子邮件地址，而不是系统生成的别名电子邮件地址。
...
以电子方式签署校样时，如果您在 [!DNL Workfront Proof] 中设置了 SSO，除了您 [!DNL Workfront] 中的电子邮件外，系统现在会检查 Federation ID。
...
**添加到校样的用户无法访问[!DNL Workfront]** 中的校样工作项

*校样*

如果将用户添加到校样工作流中的某个阶段，则该用户不会添加到“文档”共享中，并且不会获得 [!DNL Workfront] 中校样工作项的权限。 当用户在 [!DNL Workfront] 中并尝试打开附有校样的工作项时，他们会看到以下消息：

“[!UICONTROL 您的访问权限不足，无法查看此（对象）。]

此问题特定于已创建的校样和事后添加的用户。 在创建校样之前，将用户添加到工作流中可以正常工作。
...
当用户将另一个用户添加到校样时，该用户在校样上的角色设置为”[!UICONTROL 只读]“，而不管该用户的实际校样角色是什么。
...
**[!DNL XLS]和 [!DNL XLSX] 文件大小限制暂时减少到 100 MB 用于校样**

*校对*

为了解决安全问题，创建校样时，我们暂时将 [!DNL XLS] 和 [!DNL XLSX] 文件的最大文件大小限制为 100 MB。
...
**已停用和重新激活的用户不会出现在[!UICONTROL 校样联系人]**&#x200B;中
...
当用户在[!UICONTROL 文档]区域查看校样时，对校样本身的任何评论都不会显示在文档的[!UICONTROL 更新]区域中。

**[!UICONTROL 工作负载均衡器]：“[!UICONTROL ？[对象]？]” 显示在过度分配信息中**

*[!UICONTROL 工作负载平衡器]*

如果用户由于任务与用户的休息时间重叠而在[!UICONTROL 工作负载均衡器]中显示为过度分配，并且另一用户查看了他们的过度分配，则过度分配信息的“[!UICONTROL 容量]”区域显示“[!UICONTROL ？[对象]？]” 而不是用户的实际容量。
